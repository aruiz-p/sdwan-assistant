...
from webex.chat_api_client import send_message_to_chat_api
...
class AiCommand(Command):
    def __init__(self):
        super().__init__(
            command_keyword="sd-wan-assistant",
...
class AiMoreInfoCallback(Command):
    ...
    def execute(self, message, attachment_actions, activity):
        ...
        max_tokens_info_textblock = TextBlock(
            "I'm an SD-WAN AI that can run NWPI traces to help understand ongoing issues.",
            wrap=True,
            size=FontSize.SMALL,
            color=Colors.LIGHT,
        )
